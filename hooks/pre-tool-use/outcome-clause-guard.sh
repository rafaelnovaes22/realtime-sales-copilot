...
# Blocks edits to approved outcome clauses (D2.1-D2.5) without NOVAIS_FOUNDRY_BYPASS.
...
if [[ "$FILE_PATH" =~ docs/(onda-[0-9]+|clients/[^/]+)/.+\.md$ ]]; then
  if [ -f "$FILE_PATH" ] && grep -qiE "^(approved|status):\s*(true|approved|signed)" "$FILE_PATH" 2>/dev/null; then
    ...
    echo "Para editar: defina NOVAIS_FOUNDRY_BYPASS=<motivo> em settings.local.json ou no ambiente." >&2
    exit 2
  fi
fi
