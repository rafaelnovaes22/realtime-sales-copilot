...
# /novais-digital:unit-economics — C3 hard gate

## Propósito

Mede o **custo humano atual** do processo do cliente e deriva o **preço de venda mínimo** que satisfaz o hard gate **C3 (custo ≤ 25% do preço)**. Saída é input canônico para `/novais-digital:sla-threshold` e amarra a cláusula de outcome (C2) ao economics validado.

> Esta command **não estima**. Recebe dados declarados pelo cliente (idealmente coletados durante `/novais-digital:diagnose` Bloco 3). Estimativa "no chute" mata C3 silenciosamente em produção.
...
4. `__foundry_cache.offerings` disponível ou `@offerings-loader` invocável
...
# opcionais
artifact_id: <slug>           # se já passou por /novais-digital:spec, vincula
...
6. Avaliar c3_check.status:
   ...
   - unviable    — margem negativa; BLOQUEIA /novais-digital:sla-threshold
...
```yaml
command: /novais-digital:unit-economics
...
next_step: "/novais-digital:sla-threshold --artifact_id=<>" | "BLOQUEADO — renegociar"
```
...
| "C3 unviable mas vamos tentar otimizar prompt" | Otimização vem em SHADOW; antes disso, viabilidade comercial é hard gate | Bloquear `/novais-digital:sla-threshold`; `next_step: "renegociar escopo"` |
...
`/novais-digital:unit-economics` **bloqueia** `/novais-digital:sla-threshold` se:
- `c3_check.status == unviable`, OU
- `data_confidence == low` sem aprovação explícita

Reviewer DeepAgent (Foundry-3) audita mensalmente correspondência entre:
