...
# /acme:sla-threshold — Pré-contratar SLA mensurável
...
1. `docs/specs/{artifact_id}.md` existe (gerada por `/acme:spec`)
...
next_step: "/acme:plan --artifact_id=<>" | "/acme:eval --artifact_id=<>"
```
...
|---|---|---|
| "Cliente premium aceita janela <14 dias" | Hard floor C4 sem exceção; cliente que pula SHADOW é o primeiro a churnar | `min_window_days < 14` → ERROR; bypass exige `ACME_FORGE_BYPASS=incident` em settings.local (Forge-4) com log auditado |
