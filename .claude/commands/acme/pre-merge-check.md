...
# /acme:pre-merge-check — 5 gates de qualidade
...
- **Em pre-commit hook** se Forge-4 ativo (hook `pre-merge-check`)
...
**Regra**: artefatos novos/modificados/removidos no branch refletidos em `docs/forge/manifest.json`.
...
3. Para cada arquivo modificado:
   ...
   - Modificado? `sha256` no manifest está null OU desatualizado? warn (Forge-4 hook fará sync)
```
...
recommendations:
  ...
  - "G4: rodar manifest-sync (Forge-4 hook futuro) ou atualizar manifest manualmente"
...
| "G4 warning é OK, manifest atualiza no merge" | Manifest desincronizado quebra reviewer DeepAgent na próxima auditoria | Warn aceito antes de Forge-4 (hook `manifest-sync`); pós Forge-4 vira fail |
...
| "Modificar a command pra suprimir false positives específicos" | Drift de regra; cada exceção corrói C7/C8 | Falsos positivos viram issue no Forge; regra ajustada via PR e ADR |
...
## Integração com hooks (Forge-4)

Quando Forge-4 entregar o hook `pre-merge-check`, esta command será invocada automaticamente em:
- `git pre-commit` (modo lite — só G1, G2, G3 — < 5s)
- `git pre-push` (modo full — todos os 5 gates)
- CI workflow `pre-merge.yml`

Bypass via `ACME_FORGE_BYPASS=incident` em `settings.local.json` deixa rastro em `docs/forge/bypass-log/{date}.md`.
